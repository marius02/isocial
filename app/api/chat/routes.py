...
@router.get("/list", description="Get a list of chats for the authenticated user", response_model=AllChats)
...
@router.post("/", description="Create a new chat based on YouTube chat data", response_model=ChatCreateResponse)
...
@router.post("/continue", description="Continue an existing chat using provided chat data", response_model=ChatContinueResponse)
...
@router.delete("/{chat_id}/", description="Delete a chat with the specified chat ID associated with the authenticated user")
...
@router.get("/{chat_id}/", description="Retrieve details of a specific chat using its chat ID", response_model=Chat)
...
@router.post("/new-search", description="Create a new chat based on Twitter chat data for search", response_model=ChatCreateResponse)
...
@router.post("/continue-search", description="Continue an existing search-based chat using provided chat data", response_model=ChatContinueResponse)
async def continue_search(chat_data: ChatContinueData, user=Depends(current_active_user), db: AsyncSession = Depends(get_async_session), client=Depends(init_openai_client)):
    chat_repo = ChatRepository(db)
    return await chat_repo.continue_chat(user.id, client, chat_data)
